...
const updateRank = async (userId, firstName, username) => {
  try {
    ...
    const res = await pool.query(
      "SELECT xp FROM user_ranks WHERE user_id = $1",
      [userId]
    );
    ...
    await pool.query("UPDATE user_ranks SET level = $1 WHERE user_id = $2", [
      newLevel,
      userId,
    ]);
  } catch (e) {
...
// ====================================================
//       GÜVENLİ BAŞLATMA (TEK POLLING KONTROLÜ)
// ====================================================
    ...
    try {
      ...
      if (!pair) {
        return bot.sendMessage(
          chatId,
          "⚠️ *Data Syncing:* Please try again in a few minutes.",
          { parse_mode: "Markdown" }
        );
      }
...
🔗 [View on DexScreener](${pair.url})
  ...
  bot.onText(/\/rank/, async (msg) => {
    ...
    try {
      ...
      if (res.rows.length === 0) {
        ...
        return bot.sendMessage(
          msg.chat.id,
          "🆕 Profile created! Type /rank again to see stats."
        );
      }
      ...
      bot.sendMessage(
        msg.chat.id,
        "⚠️ Database syncing. Try again shortly."
      );
    }
  });
  ...
  bot.onText(/\/ask (.+)/, async (msg, match) => {
    ...
    if (openai) {
      ...
      try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are Hyper Logic AI ($SKYL). Professional, futuristic, strict English. Concise answers.",
            },
        ...
        bot.sendMessage(
          chatId,
          "⚠️ AI overloaded. Accessing offline database...",
          { parse_mode: "Markdown" }
        );
  ...
  const handleOfflineAI = (chatId, question) => {
    ...
    const knowledgeBase = [
      {
        keys: ["contract", "ca", "address"],
        answer: `💎 *Contract:* \`${TOKEN_CA}\``,
      },
      {
        keys: ["buy", "pancake", "swap"],
        answer: `🛒 *Buy here:* [PancakeSwap](${BUY_LINK})`,
      },
      {
        keys: ["airdrop", "claim"],
        answer: `🎁 *Claim here:* ${AIRDROP_PAGE}`,
      },
      {
        keys: ["tax", "slippage"],
        answer: `💸 *Tax:* 0% Buy / 0% Sell (Low slippage)`,
      },
      {
        keys: ["lock", "lp"],
        answer: `🔒 *Liquidity:* Locked for 5 Years.`,
      },
      {
        keys: ["roadmap", "plan"],
        answer: `🗺 *Roadmap:* Launch → AI Dashboard → Staking → Tier 1 CEX.`,
      },
    ];
    const found = knowledgeBase.find((item) =>
      item.keys.some((k) => lowerQ.includes(k))
    );
    const resp = found
      ? found.answer
      : `🤖 I am strictly an AI Analyst. For details, visit: ${WEBSITE}`;
  ...
  bot.onText(/\/chart/, (msg) => {
    ...
    bot.sendMessage(msg.chat.id, "📈 *Live Charts*", {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: "🦅 DexScreener", url: `https://dexscreener.com/bsc/${pair}` }],
        ],
      },
    });
  });
...
📸 [Instagram](https://www.instagram.com/skyline.logic)
  ...
  bot.onText(/\/raid (.+)/, async (msg, match) => {
    ...
    if (msg.chat.type === "private")
      return bot.sendMessage(chatId, "⚠️ Group only.");

    if (!url.includes("twitter.com") && !url.includes("x.com")) {
      return bot.sendMessage(
        chatId,
        "❌ Invalid raid link. Use Twitter/X links."
      );
    }
    ...
    try {
      ...
      await bot.sendPhoto(chatId, IMG_RAID, {
        ...
        parse_mode: "Markdown",
        reply_markup: {
          inline_keyboard: [[{ text: "⚔️ ATTACK NOW ⚔️", url }]],
        },
      });
    } catch (e) {
      console.error("Raid Cmd Error:", e.message);
    }
  });
  ...
  bot.onText(/\/announce(?:\s+([\s\S]+))?/, async (msg, match) => {
    ...
    if (!content) {
      return bot.sendMessage(
        chatId,
        "⚠️ Usage: `/announce Your Message Here`",
        { parse_mode: "Markdown" }
      );
    }
    ...
    try {
      ...
      console.error("Announce Cmd Error:", e.message);
    }
  });
  ...
  bot.on("message", async (msg) => {
    ...
    if (lower.includes("moon") || lower.includes("lambo")) {
      bot.sendMessage(msg.chat.id, "🚀 *To the Sky!*", {
        parse_mode: "Markdown",
      });
    }
  });
  ...
  bot.on("new_chat_members", async (msg) => {
    ...
    for (const member of msg.new_chat_members) {
      ...
      try {
        ...
        console.error("Restrict on join error:", e.message);
      }
      ...
      const opts = [
        ...
        {
          text: `${n1}+${n2}=${ans + 1}`,
          callback_data: `cap|no|${member.id}`,
        },
  ...
  bot.on("callback_query", async (q) => {
    ...
    if (status === "ok") {
      try {
        ...
        console.error("Unrestrict on captcha error:", e.message);
      }
  ...
  bot.on("left_chat_member", async (msg) => {
    ...
    try {
      await bot.sendPhoto(chatId, IMG_GOODBYE, {
        caption: goodbyeCaption,
        parse_mode: "Markdown",
      });
    } catch (e) {
      bot.sendMessage(chatId, goodbyeCaption, { parse_mode: "Markdown" });
    }
  });
}
...
// ====================================================
export const sendBuyDetected = async (
  amountSKYL,
  costWBNB,
  wallet,
  txHash,
...
<a href="https://bscscan.com/tx/${escape(txHash)}">View on BscScan</a>`.trim();
  ...
  try {
    await bot.sendPhoto(CHAT_ID, finalImg, { caption: txt, parse_mode: "HTML" });
  } catch (e) {
    console.error("Buy Alert Error:", e.message);
  }
};
